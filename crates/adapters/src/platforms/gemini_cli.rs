...
impl HookAdapter for GeminiCliAdapter {
    ...
    fn platform_id(&self) -> PlatformId {
        PlatformId::GeminiCli
    }
}

impl BaseAdapter for GeminiCliAdapter {
    fn session_dir_segments(&self) -> Vec<String> {
        get_session_dir_segments(PlatformId::GeminiCli).unwrap_or_default()
    }
    ...
    fn check_plugin_registration(&self) -> DiagnosticResult {
        ...
        DiagnosticResult {
            check: "gemini-cli config directory".to_string(),
            status: if exists {
            ...
            fix: if exists {
                ...
                Some("Install Gemini CLI and run 'gemini' to initialize configuration.".to_string())
            },
        }
    }
}
