...
pub struct JetbrainsCopilotAdapter;

impl HookAdapter for JetbrainsCopilotAdapter {
    ...
    fn platform_id(&self) -> PlatformId {
        PlatformId::JetbrainsCopilot
    }
}

impl BaseAdapter for JetbrainsCopilotAdapter {
    fn session_dir_segments(&self) -> Vec<String> {
        get_session_dir_segments(PlatformId::JetbrainsCopilot).unwrap_or_default()
    }
    ...
    fn check_plugin_registration(&self) -> DiagnosticResult {
        ...
        DiagnosticResult {
            check: "jetbrains-copilot config directory".to_string(),
            status: if exists {
            ...
            fix: if exists {
                ...
                Some(
                    "Install JetBrains IDE with GitHub Copilot plugin to initialize configuration."
                        .to_string(),
                )
            },
        }
