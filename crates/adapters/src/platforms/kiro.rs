...
impl HookAdapter for KiroAdapter {
...
impl BaseAdapter for KiroAdapter {
    fn session_dir_segments(&self) -> Vec<String> {
        get_session_dir_segments(PlatformId::Kiro).unwrap_or_default()
    }
    ...
    fn check_plugin_registration(&self) -> DiagnosticResult {
        ...
        DiagnosticResult {
            check: "kiro config directory".to_string(),
            status: if exists {
            ...
            fix: if exists {
                ...
                Some("Install Kiro and run it to initialize configuration.".to_string())
            },
        }
    }
}
