...
impl HookAdapter for VscodeCopilotAdapter {
    ...
    fn platform_id(&self) -> PlatformId {
        PlatformId::VscodeCopilot
    }
}

impl BaseAdapter for VscodeCopilotAdapter {
    fn session_dir_segments(&self) -> Vec<String> {
        get_session_dir_segments(PlatformId::VscodeCopilot).unwrap_or_default()
    }
    ...
    fn check_plugin_registration(&self) -> DiagnosticResult {
        ...
        DiagnosticResult {
            check: "vscode-copilot config directory".to_string(),
            status: if exists {
            ...
            fix: if exists {
                ...
                Some(
                    "Install VS Code with GitHub Copilot extension to initialize configuration."
                        .to_string(),
                )
            },
        }
