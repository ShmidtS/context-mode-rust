...
impl HookAdapter for QwenCodeAdapter {
...
impl BaseAdapter for QwenCodeAdapter {
    fn session_dir_segments(&self) -> Vec<String> {
        get_session_dir_segments(PlatformId::QwenCode).unwrap_or_default()
    }
    ...
    fn check_plugin_registration(&self) -> DiagnosticResult {
        ...
        DiagnosticResult {
            check: "qwen-code config directory".to_string(),
            status: if exists {
            ...
            fix: if exists {
                ...
                Some("Install Qwen Code and run it to initialize configuration.".to_string())
            },
        }
    }
}
