...
impl HookAdapter for ZedAdapter {
...
impl BaseAdapter for ZedAdapter {
    fn session_dir_segments(&self) -> Vec<String> {
        get_session_dir_segments(PlatformId::Zed).unwrap_or_default()
    }
    ...
    fn check_plugin_registration(&self) -> DiagnosticResult {
        ...
        DiagnosticResult {
            check: "zed config directory".to_string(),
            status: if exists {
            ...
            fix: if exists {
                ...
                Some("Install Zed editor and open it to initialize configuration.".to_string())
            },
        }
    }
}
