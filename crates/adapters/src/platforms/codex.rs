...
impl BaseAdapter for CodexAdapter {
    fn session_dir_segments(&self) -> Vec<String> {
        get_session_dir_segments(PlatformId::Codex).unwrap_or_default()
    }
    ...
    fn check_plugin_registration(&self) -> DiagnosticResult {
        ...
        DiagnosticResult {
            check: "codex config directory".to_string(),
            status: if exists {
            ...
            fix: if exists {
                ...
                Some("Install Codex CLI and run 'codex' to initialize configuration.".to_string())
            },
        }
    }
}
