...
impl BaseAdapter for CursorAdapter {
    fn session_dir_segments(&self) -> Vec<String> {
        get_session_dir_segments(PlatformId::Cursor).unwrap_or_default()
    }
    ...
    fn check_plugin_registration(&self) -> DiagnosticResult {
        ...
        DiagnosticResult {
            check: "cursor config directory".to_string(),
            status: if exists {
            ...
            fix: if exists {
                ...
                Some("Install Cursor and open it to initialize configuration.".to_string())
            },
        }
    }
}
