...
Use: `mcp__context-mode__ctx_fetch_and_index(url, source)` or `mcp__context-mode__ctx_execute(language: "python", code: "...")`

### Inline HTTP — FORBIDDEN
No inline HTTP one-liners (`fetch`, `requests.get`, `http.get`) via `run_command`. Bypasses sandbox.
...
`run_command` ONLY for: `git`, `mkdir`, `rm`, `mv`, `cd`, `ls`, `cargo build`, `pip install`.
...
- `mcp__context-mode__ctx_batch_execute(commands: [{ label: "cmd1", command: "gh pr list" }, { label: "cmd2", command: "curl -s https://api.example.com/v1/status" }], queries: ["status"])` — gh, curl, dig, docker inspect, multi-region cloud queries
