...
Terminal ONLY for: `git`, `mkdir`, `rm`, `mv`, `cd`, `ls`, `cargo build`, `pip install`.
...
Pass `concurrency: 4-8` to `ctx_batch_execute` and `ctx_fetch_and_index` for network/API batches. Keep `concurrency: 1` for CPU-bound work (cargo test, build, clippy). GitHub gh: cap at 4.
